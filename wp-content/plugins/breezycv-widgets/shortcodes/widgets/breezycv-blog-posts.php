...
class Breezycv_Blog_Posts extends Widget_Base {

	public function get_name() {
		return 'breezycv-blog-posts';
	}

	public function get_title() {
		return __( 'Blog Posts', 'breezycv-widgets' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'layout',
			[
				'label' => __( 'Layout', 'breezycv-widgets' ),
				'type' => \Elementor\Controls_Manager::SELECT,
				'default' => 'two-columns',
				'options' => [
					'one-column'  => __( 'One Column', 'breezycv-widgets' ),
					'two-columns' => __( 'Two Columns', 'breezycv-widgets' ),
					'three-columns' => __( 'Three Columns', 'breezycv-widgets' ),
				],
			]
		);
		...
		$this->add_control(
			'number_of_posts',
			[
				'label'       => __( 'Number of posts to show', 'breezycv-widgets' ),
				'type'        => Controls_Manager::TEXT,
				'placeholder' => __( 'Type number here', 'breezycv-widgets' ),
				'label_block' => true,
				'default' 	   => __( '6', 'breezycv-widgets' ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'label_on' => __( 'On', 'breezycv-widgets' ),
				'label_off' => __( 'Off', 'breezycv-widgets' ),
				'return_value' => 'yes',
				'default' => 'no',
			]
		);
	...
	protected function render() {
		                                        ...
		                                        <?php
		                                            ...
		                                            if ( ! empty( $categories ) ) {
		                                                foreach( $categories as $category ) {
		                                                    $output .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . esc_attr( sprintf( esc_html__( 'View all posts in %s', 'breezycv-widgets' ), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>' . $separator;
		                                                }
		...
		<?php
		    ...
		    {
		        ?>
		            <a class="btn btn-primary" href="<?php echo esc_url(home_url()); ?>"><?php echo esc_html_e( 'See All Posts', 'breezycv-widgets' ); ?></a>
		        <?php
		    }
		    else
		    {
		        ...
		        ?>
		            <a class="btn btn-primary" href="<?php echo esc_url($blog_page_url); ?>"><?php echo esc_html_e( 'See All Posts', 'breezycv-widgets' ); ?></a>
		        <?php
		    }
	}
