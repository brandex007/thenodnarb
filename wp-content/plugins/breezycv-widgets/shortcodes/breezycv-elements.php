...
class Breezycv_Elements
{
    ...
    {
        \Elementor\Plugin::instance()->elements_manager->add_category( 'breezycv-elements', array(
            'title' => __( 'BreezyCV Elements', 'breezycv-widgets' ),
        ), 1 );
    }
    ...
    {
		wp_enqueue_style( 'breezycv_widget_styles', plugins_url( 'assets/css/style.css', __FILE__ ), array(), '1.0.0');
    }

    // Register Widget Scripts
    public function register_widget_scripts()
    {
		wp_enqueue_script( 'breezycv_scripts', plugins_url( 'assets/js/scripts.js', __FILE__ ), array( 'jquery' ), true, 1, 'all' );
        $theme_style = ( function_exists('fw_get_db_settings_option') ) ? fw_get_db_settings_option('theme_style') : 'light';
        if ($theme_style == 'dark') {
            wp_enqueue_style( 'breezycv_widget_styles_dark', plugins_url( 'assets/css/dark-styles.css', __FILE__ ), array(), '1.0.0');
        }
        if ( is_rtl() ) {
            wp_enqueue_style('breezycv-widgets-rtl-styles', plugins_url( 'assets/css/rtl.css', __FILE__ ), array(), '1.0.0');
        }
    }
    ...
    private function include_widget($file)
    {
		...
		$class = sprintf( 'Breezycv\Widgets\%s', $class );
